...
const CONFIG = {
  ...
  ALLOWED_ROLES: [
    // الرولات اللي مسموح لها تستخدم أوامر السكريم
    "1363502731305943171",
    "1363502730227875910",
    "1363502726939414588",
    "1363502725698027660",
...
commands.push(
  new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt
        .setName("time")
        .setDescription("Time/slot (optional)")
        .setRequired(false),
    ),
);
commands.push(
...
commands.push(
  new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt.setName("time").setDescription("Time/slot").setRequired(true),
    ),
);
commands.push(
  new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt.setName("time").setDescription("Time").setRequired(true),
    ),
);
commands.push(
  new SlashCommandBuilder()
    .setName("fb")
    .setDescription("Send feedback message"),
);
commands.push(
  new SlashCommandBuilder()
    .setName("cancel_reg")
    .setDescription("إلغاء التسجيل للسكريم"),
);
...
["era", "mir", "san"].forEach((map) => {
  commands.push(
    new SlashCommandBuilder()
      .setName(map)
      .setDescription(
        `${map.toUpperCase()} map announcement (sends DM to mentioned role)`,
      )
      .addStringOption((opt) =>
        opt.setName("id").setDescription("Room ID").setRequired(true),
      )
      .addStringOption((opt) =>
        opt.setName("pass").setDescription("Password").setRequired(true),
      )
      .addStringOption((opt) =>
        opt.setName("wait").setDescription("Wait time").setRequired(true),
      )
      .addRoleOption((opt) =>
        opt
          .setName("mention")
          .setDescription("Role to mention and DM")
          .setRequired(true),
      ),
  );
});
...
commands.push(
  new SlashCommandBuilder()
    .setName("mute")
    .setDescription("Mute member (adds Muted role)")
    .addUserOption((o) =>
...
commands.push(
  new SlashCommandBuilder()
    .setName("lock")
    .setDescription("Lock this text channel"),
);
commands.push(
  new SlashCommandBuilder()
    .setName("unlock")
    .setDescription("Unlock this text channel"),
);
commands.push(
  new SlashCommandBuilder()
    .setName("hide")
...
commands.push(
  new SlashCommandBuilder()
    .setName("unhide")
    .setDescription("Show this channel to everyone"),
);
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((opt) =>
          opt
            .setName("command")
            .setDescription("Command name (optional)")
            .setRequired(false),
        ),
    ),
);
...
commands.push(
  new SlashCommandBuilder()
    ...
    .addUserOption((opt) =>
      opt.setName("user").setDescription("العضو | User").setRequired(false),
    ),
);
...
commands.push(
  new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt
        .setName("time")
        .setDescription("الوقت (مثال: 5m, 1h, 1d) | Time (e.g: 5m, 1h, 1d)")
        .setRequired(true),
    )
    .addStringOption((opt) =>
      opt
        .setName("message")
        .setDescription("رسالة التذكير | Reminder message")
        .setRequired(true),
    ),
);
...
// لوج الرسايل المحذوفه
//client.on("messageDelete", (message) => {
 // if (message.author && !message.author.bot)
 //   logToChannel(
  //    message.guild,
   //   `🗑 Message deleted in ${message.channel}: ${message.author.tag}: ${message.content}`,
  //  );
// });
client.on("messageUpdate", (oldMsg, newMsg) => {
  if (oldMsg.author && !oldMsg.author.bot)
    logToChannel(
      oldMsg.guild,
      `✏️ Message edited in ${oldMsg.channel}: ${oldMsg.author.tag}\nBefore: ${oldMsg.content}\nAfter: ${newMsg.content}`,
    );
});
        ...
        .addOptions([
          {
            ...
            value: "scrim",
            description:
              "إدارة التسجيلات والمباريات | Registration & match management",
          },
          {
            label: "🛡️ الإشراف | Moderation",
            value: "moderation",
            description:
              "أوامر الإشراف والعقوبات | Moderation & punishment commands",
          },
          {
            label: "📢 الخرائط | Maps",
            value: "maps",
          ...
          },
          {
            ...
            value: "permissions",
            description:
              "تخصيص صلاحيات الأوامر | Command permissions management",
          },
          {
          ...
          },
          {
            ...
            value: "advanced",
            description:
              "الإحصائيات والتذكيرات والمميزات المتقدمة | Stats, reminders & advanced features",
          },
        ]);
...
• Admins have full access to all commands
    ...
    if (["reg", "close_reg", "spare", "anno", "fb"].includes(cmd)) {
      if (!hasPermission)
        return interaction.reply({
          content: "❌ مش مسموحلك تستخدم الأمر ده.",
          ephemeral: true,
        });
    }
    ...
    if (cmd === "reg") {
      ...
      if (interaction.member.roles.cache.has(CONFIG.BLACKLIST_ROLE_ID)) {
        return interaction.reply({
          content:
            "❌ **أنت محظور من التسجيل في السكريمات**\n❌ **You are banned from registering in scrims**",
          ephemeral: true,
        });
      }
   ...
   > https://discord.com/channels/1357088796276949122/1363504050754031746
    ...
    if (cmd === "cancel_reg") {
      ...
      logToChannel(
        interaction.guild,
        `⚠️ Scrim cancelled by ${interaction.user.tag}`,
      );
      return;
    }
...
🔗 **رابط الرسالة:** ${messageLink}`;
    ...
    if (cmd === "blacklist") {
      ...
      if (sub === "add") {
        ...
        await interaction.reply({
          content: `✅ Added ${user.tag} to blacklist`,
          ephemeral: true,
        });
      } else {
        await member.roles.remove(role);
        await interaction.reply({
          content: `✅ Removed ${user.tag} from blacklist`,
          ephemeral: true,
        });
      }
      return;
    ...
    if (cmd === "unhide") {
      ...
      await interaction.reply({
        content: "👁️ Channel visible to everyone",
        ephemeral: true,
      });
      return;
    }
    ...
    if (cmd === "perms") {
      ...
      if (sub === "add") {
        ...
        if (success) {
          await interaction.reply({
            content: `✅ Added role ${role.name} to command /${commandName}`,
            ephemeral: true,
          });
        } else {
          await interaction.reply({
            content: `❌ Role ${role.name} already has permission for /${commandName}`,
            ephemeral: true,
          });
        }
      } else if (sub === "remove") {
        ...
        if (success) {
          await interaction.reply({
            content: `✅ Removed role ${role.name} from command /${commandName}`,
            ephemeral: true,
          });
        } else {
          await interaction.reply({
            content: `❌ Role ${role.name} doesn't have permission for /${commandName}`,
            ephemeral: true,
          });
        }
      } else if (sub === "list") {
    ...
    if (cmd === "userinfo") {
      ...
      if (!member) {
        return interaction.reply({
          content: "❌ العضو غير موجود في السيرفر | User not found in server",
          ephemeral: true,
        });
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          },
          {
            ...
            value:
              roles.length > 0 ? roles.join(", ") : "لا توجد رولات | No roles",
            inline: false,
          },
        )
        .setFooter({
          text: isBlacklisted
            ? "⚠️ هذا العضو محظور | This user is blacklisted"
            : `طلب بواسطة ${interaction.user.tag}`,
          iconURL: interaction.user.displayAvatarURL(),
        })
        .setTimestamp();
    ...
    if (cmd === "remind") {
      ...
      switch (unit) {
        ...
        default:
          return interaction.reply({
            content: "❌ وحدة وقت غير صالحة | Invalid time unit",
            ephemeral: true,
          });
      }
      ...
      if (milliseconds > 7 * 24 * 60 * 60 * 1000) {
        ...
        return interaction.reply({
          content:
            "❌ الحد الأقصى للتذكير 7 أيام | Maximum reminder time is 7 days",
          ephemeral: true,
        });
      }
